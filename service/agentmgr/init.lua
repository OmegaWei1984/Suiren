...
s.resp.relogin = function(source, playerid, node, gate)
    local mplayer = players[playerid]
    if mplayer and mplayer.status == STATUS.LOGOUT then
        skynet.error("relogin fail as status LOGOUT " .. playerid)
        return false
    end
    if mplayer and mplayer.status == STATUS.LOGIN then
        skynet.error("relogin fail as status LOGIN " .. playerid)
        return false
    end
    if mplayer then
